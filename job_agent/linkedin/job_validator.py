...
class JobValidator:
    def __init__(self, cv_path:str="data/resume.pdf"):
        self.cv_summary = extract_pdf_text(cv_path)
    ...
    def get_job_facts(self, job_details: dict) -> Optional[dict]:
        ...
        json_schema = {
            ...
            "red_flags": ["list", "of", "red", "flags", "found", "in", "text"]
        }
    ...
    def validate_job(self, job_detail,  job_data: dict, validation_data:dict):
        # 1. validate years of experience required
        validations = {}
        is_valid = isinstance(job_data["experience_min"], int) and job_data["experience_min"] <= validation_data["experience_years"]
        validations['experience_years'] = {
            'is_valid': is_valid,
            'job_min_experience_years': job_data["experience_min"]
        }
        
        # 2. validate the work mode
        is_work_mode_valid = job_data['work_model'] == validation_data['work_model'] or job_data['relocation_offered']
        validations['work_model'] = {
            'is_valid': is_work_mode_valid,
            'job_work_model': job_data['work_model'],
            'relocation_offered': job_data['relocation_offered']
        }
        
        # 3. validate the job salary range
        is_salary_valid = isinstance(job_data['salary_max'], int) and job_data['salary_max'] <= validation_data['max_salary']
        validations = {
            'is_valid': is_salary_valid,
            'job_max_salary': job_data['salary_max']
        }
        
        # 4. validate the geographic restrictions
        is_geo_valid = job_data["is_geography_valid"]
        validations={
            "is_geo_valid": is_geo_valid,
            "geographic_restrictions": job_data["geographic_restrictions"]
        }
        
        # 5. validate the number of people applied
        is_applicant_count_valid = isinstance(job_detail['applicants_count'], int) and job_detail['applicants_count'] <= validation_data['max_applicants_count']
        validations={
            "is_applicant_count_valid": is_applicant_count_valid,
            "applicants_count": job_detail['applicants_count']
        }
...
if __name__ == "__main__":
    ...
    result = job_validator.get_job_facts(data)
    validation_data = {
        'experience_years': 3,
        'is_remote': True,
        'max_salary': 150000,
    ...
    if result:
        validations = job_validator.validate_job(data, result, validation_data)